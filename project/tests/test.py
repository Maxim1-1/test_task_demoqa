...
def test(set_up):
    ...
    assert textbox_page.is_validate_output_full_name(random_text), f'Значение full name не соответствует ожидаемому.Ожидаемое значение: {random_text}, актуальное значение: {full_name_actual_value}'

    email_actual_value = textbox_page.get_output_output_email()
    assert textbox_page.is_validate_output_email(random_email), f'Значение e-mail не соответствует ожидаемому.Ожидаемое значение: {random_text}, актуальное значение: {email_actual_value}'

    current_address_actual_value = textbox_page.get_output_current_address()
    assert textbox_page.is_validate_current_address(random_text), f'Current address не соответствует ожидаемому.Ожидаемое значение: {random_text}, актуальное значение: {current_address_actual_value}'

    permanent_address_actual_value = textbox_page.get_output_permanent_address()
    assert textbox_page.is_validate_permanent_address(random_text), f'Permanent address full name не соответствует ожидаемому.Ожидаемое значение: {random_text}, актуальное значение: {permanent_address_actual_value}'
    ...
    current = alerts_frame_windows_page.get_current_tab()
    alerts_frame_windows_page.new_button_click()
    alerts_frame_windows_page.switch_to_new_tab(current)
