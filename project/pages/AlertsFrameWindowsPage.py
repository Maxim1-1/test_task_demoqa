...
class AlertsFrameWindowsPage(BasePage):
    ...
    ALERTS_FRAME_WINDOW_OPEN = "//*[contains(text(),'Alerts, Frame & Windows')]/../../following::*[contains(@class,'collapse show')]"
    ...
    def alerts_frame_windows_click(self):
        BaseElement(By.XPATH, self.ALERTS_FRAME_WINDOW_OPEN).wait_element()
        BaseElement(By.XPATH, self.ALERTS_FRAME_WINDOW).find_element().click()


    def is_validate_open_new_tab(self):
        if BaseElement(By.XPATH, self.NEW_TAB_TEXT).find_element():
            return True
        else:
            return False
